"""
MCLeadProspecta Web — by Memocash Soluções
Versão 0.0.0.1 pré-beta  |  github.com/laossim
"""

import os, re, time, uuid, json, threading, unicodedata, queue
...
JOBS = {}          # job_id → { status, log, progress, file_path, total }
...
def salvar_xlsx(resultados, cidade, nicho):
    ...
    wb = Workbook()
    C_H_BG="1e8a2e"; C_H_FT="FFFFFF"; C_PAR="eaf5eb"; C_IMPAR="FFFFFF"
    ...
    c=ws["A1"]; c.value=f"LEADS — {nicho.upper()} EM {cidade.upper()}"
    c.font=Font(name="Arial",bold=True,size=14,color=C_TITU)
    c.alignment=Alignment(horizontal="center",vertical="center")
    c.fill=PatternFill("solid",fgColor="d4edda"); ws.row_dimensions[1].height=32

    ws.merge_cells("A2:H2")
    c2=ws["A2"]
    c2.value=(f"Gerado em {date.today().strftime('%d/%m/%Y')}  ·  "
              f"{len(resultados)} estabelecimentos  ·  MCLeadProspecta by Memocash Soluções")
    ...
    for idx,r in enumerate(resultados,1):
        lin=idx+4; cor_bg=C_PAR if idx%2==0 else C_IMPAR
    ...
    dv.sqref=f"F5:F{total}"; ws.add_data_validation(dv)
    dp=DataValidation(type="list",formula1='"Alta 🔴,Média 🟡,Baixa 🟢,VIP ⭐"',
                      allow_blank=True,showDropDown=False)
    dp.sqref=f"H5:H{total}"; ws.add_data_validation(dp)
    ...
    c=ws3["A1"]; c.value="RESUMO DO FUNIL DE VENDAS"
    c.font=Font(name="Arial",bold=True,size=13,color="FFFFFF")
    c.fill=PatternFill("solid",fgColor="1e8a2e")
    c.alignment=Alignment(horizontal="center",vertical="center"); ws3.row_dimensions[1].height=28
    metricas=[
    ...
    for i,((label,formula),cor) in enumerate(zip(metricas,cores_r),2):
        ...
        cl.border=Border(bottom=Side(style="thin",color="FFFFFF"))
...
# ──────────────────────────────────────────────────────────────────────────────
# WORKER  (roda em thread separada por job)
# ──────────────────────────────────────────────────────────────────────────────
        ...
        with sync_playwright() as p:
            ...
            for _ in range(14):
                ...
                if page_lista.locator("span:has-text('Você chegou ao fim da lista')").count()>0:
                    log("  ↳ Fim da lista."); break

            elementos = page_lista.locator("a[href*='/place/']")
            nomes_vistos = set(); estabs = []
            ...
            log(f"✅ {total_enc} locais encontrados. Coletando detalhes...")
            ...
            for idx,est in enumerate(estabs,1):
                ...
                resultados.append({"nome":est["nome"],"telefone":det["telefone"],
                                   "whatsapp":det["whatsapp"],"endereco":det["endereco"],
                                   "url":est["url"]})
            ...
            log(f"\n✅ Concluído!")
            log(f"   Total de leads : {len(resultados)}")
            log(f"   Com telefone   : {com_tel}")
            log(f"   Arquivo        : {nome_arq}")
...
def rota_iniciar():
    ...
    t = threading.Thread(target=worker, args=(job_id, cidade, nicho), daemon=True)
    t.start()
...
def rota_download(job_id):
    ...
    return send_file(job["file_path"],
                     as_attachment=True,
...
def rota_historico():
    with JOBS_LOCK:
        hist = [
            {"cidade":j["cidade"],"nicho":j["nicho"],
             "status":j["status"],"total":j["total"],
             "criado_em":j["criado_em"],"job_id":jid}
